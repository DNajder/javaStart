...
//        Person person1 = new Person(); - nie zadziała na klasie abstrakcyjnej Person!
            ...
            Pensoner pensoner = new Pensoner("Janusz", "Kwiatkowski", 912, 91, BigDecimal.valueOf(800));

            Person[] people = new Person[]{worker, student, pensoner};
            Object[] objects = new Object[]{worker, student, pensoner};

            List<Person> peopleList = new ArrayList<>();
            peopleList.add(worker);
            peopleList.add(student);
            peopleList.add(pensoner);

            printDataLegacyMode(worker, student, pensoner);
...
//        nameHolders.add(pensioner); - nie implementuje tego interfejsu
        ...
        private static void printDataLegacyMode(Worker worker, Student student, Pensoner pensioner) {
        ...
        private static void printDataProperWay(List<Person> peopleInfo) {
            for (Person person : peopleInfo) {
                ...
                if (person instanceof Worker) {
                    ...
                    ((Pensoner) person).introductionInner();
                }
            }
        }
    }
